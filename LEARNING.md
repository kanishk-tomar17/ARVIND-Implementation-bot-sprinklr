# GROOT — Learning from Lyearn & the Sprinklr platform

How GROOT studies the foundational courses and the live product, using your own logged-in browser. **No passwords are shared** — GROOT attaches to a Chrome session *you* have already logged into.

## One-time: let GROOT attach to your Chrome

GROOT uses the `chrome-devtools` MCP (see `.mcp.json`), which connects to a Chrome started with remote debugging.

1. Double-click **`Start-GROOT-Chrome.bat`** on your Desktop. It closes Chrome and reopens it with debugging on, using a **dedicated GROOT profile** (`C:\GROOT-Chrome-Profile`).
...
That's it. GROOT now sees the tabs in the GROOT profile. (Your normal Chrome profile is untouched and separate.)

## How GROOT learns a course
...
With your Sprinklr environment open, GROOT can navigate real config screens to learn exact navigation paths, field names, and behaviour — then bake those into the KB and into diagnoses. It treats your environment as read-only while learning; it follows the `sprinklr-takeover` safety rules before changing anything.

## Honest limit
GROOT reads **text**. A video with spoken narration and **no captions/transcript** can't be consumed by any tool here — those courses get flagged as gaps, and GROOT falls back to `sprinklr.com/help` for that topic.
