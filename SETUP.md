...
One-time setup to give GROOT its browser hands and confirm its knowledge sources. Do this on the consultant's machine.

---

## 1. Browser MCP (inspection, learning + automation, inside Claude Code)

Lets GROOT navigate Chrome, read config/DOM and transcript text, and drive the UI from the CLI — attaching to **your own logged-in Chrome** so no passwords are shared. Config is already in `.mcp.json` (`chrome-devtools`).
...
Manual one-time steps (GROOT can't install this for you):
...
5. When GROOT escalates to takeover, drive the fix through the extension in that tab.
...
- **`sprinklr.com/help`:** GROOT uses WebFetch/WebSearch — no setup, just internet access.
...
Quick check after setup: ask GROOT "How do assignment rules work in Unified Routing?" — it should answer from the KB / `sprinklr.com/help` with a source link.
