...
| 11 | **Never self-fill a mandatory field — open the screen, read the real layout/options, then ask.** For **every** mandatory field in a build/takeover (capacity profile, work queue, rule, dashboard, etc.): (a) do **not** fill it yourself, even with a "sensible default"; (b) **open the form/screen first** so you have the actual layout, and for picklists/dropdowns **read the real available options** from the live UI; (c) **present those options to the consultant and ask what to put** (you may recommend, but they choose). Only proceed once they've answered. Ask **up front** — enumerate all mandatory fields at the start, don't discover blockers mid-flow. (Generalizes #5/#10 to *all* mandatory fields and to the *start* of the task; the name field on the capacity profile should have been asked, not defaulted.) | Kanishk Tomer | 2026-07-03 | ✅ applied |
