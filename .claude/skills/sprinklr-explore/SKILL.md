...
2. **Step through ALL tabs / wizard steps — never document only step 1.** E.g. a queue = *General Settings → Routing Configuration → Assignees*; an agent edit = *Set Skills → Capacity → Voice Settings*; a capacity profile = *Capacity Profile → Share Settings*. Click each tab and capture it.
